	...
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if($isOrderPlaced)
		{
            ...
            if (!$USER->IsAuthorized()) {
                ...
                //var_dump( $arUserResult["ORDER_PROP"]["3"]);
                //die();
                ...
                } elseif(isset($arUserResult["ORDER_PROP"]["3"])) {
                    $user = UserTable::getList([
                        'select' => [
                            'ID'
                        ],
                        'filter' => [
                            'PERSONAL_PHONE' => $arUserResult["ORDER_PROP"]["3"]
                        ]
                    ])->fetch();
                    if (isset($user['ID'])) {
                        $arResult["ERROR"][] = "Пользователь с таким телефоном уже существует. Пожалуйста, авторизуйтесь";
                    }
                }
            }
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
			 if(isset($arUserResult["ORDER_PROP"]["3"])) {
			     $PHONE_USER = $arUserResult["ORDER_PROP"]["3"];
			 }
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
